        ...
        for term in terms['terms']:
            if term['used']:
                ...
                if not company_found:
                    ...
                    return {"result": None, "errors": err_msg_list}
            else:
                if term['type'] == 'skill':
                    ...
                    master_resume['skills']['hard_skills'] = [
                        skill for skill in master_resume['skills']['hard_skills']
                        if skill['term'] != term['term']
                    ]
                    master_resume['skills']['soft_skills'] = [
                        skill for skill in master_resume['skills']['soft_skills']
                        if skill['term'] != term['term']
                    ]
                else:
                    ...
                    master_resume['keywords'] = [
                        keyword for keyword in master_resume['keywords']
                        if keyword['term'] != term['term']
                    ]
        return {"result": master_resume, "errors": err_msg_list}
    except Exception as e:
        return {"result": None, "errors": [str(e)]}

demo = gr.Interface(
    fn=merge_jsons,
    inputs=[gr.JSON(label="master_resume"), gr.JSON(label="terms")],
    outputs=gr.JSON(label="result"),
    description="Merge two JSONs for CVTailorAI. Returns {'result': ..., 'errors': ...}"
)
